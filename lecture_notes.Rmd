...
## First, get all lines where the first character is NOT ">", (-v specifies line) then every line that contains caracters that are not ACGTacgt. -n specifies that we want to return the number of times each character occurs as well.
...
* `del ll`
    * delete element of `ll`: `del ll[0]` would remove first element
* `.extend([x])`
    *
* `.insert(i,x)`
* `.reverse()`
* `.pop()`
    * removes last element by default. Can also be used to remove any element: `ll.pop(1)` removes the second element
* `.sort()`
    * will sort the list, and then save to element
...
For lists, `* 2` means repeat twice:
...
```{python}
print([num+5 for num in count])
```

To modify values in place, use loop.

#### Regular expressions in python

Tools for simple patterns
...
To see all functions that can operate on string, use `dir("")`:

```{python}
print(dir(""))
```

For regular expressions, use the library `re` and its functions `re.search`, `re.findall`, `re.sub`, `re.split`. 

```{python}
import glob
filenames = glob.glob('../swc-python/data/*.csv')
...
Look for filenames that do NOT start with an i:
...
Abbreviate genus name to its first letter, and replace space by underscore:
...
## Loop over taxa...
for taxon in taxa:
    ## ... and for each taxa, append the result where spaces are substituted with _,
    ##     and the first word is replaces with the first letter.
    taxa_abbrev.append(
...
print(re.split(r's.p', coolstring))
print(re.split(r's.*p', coolstring))
print(re.split(r's.*?p', coolstring))
```
...
SWC sections 6-9. A few pointers:

* If the first thing written in a functon is a string not assigned to a value, that string is attached to the function as documentation, and can be returned using `help`:

```{python}
def fahr_to_cels(temp):
  '''
  Takes a temperature measured in fahrenheit, and returns that temperature in celsius.
  '''
  return(5*(temp-32)/9)
  
print(fahr_to_cels(70))


help(fahr_to_cels)  
```

* see solutions [here](./python_homework)
